...
TFT_eSprite menuSpr = TFT_eSprite(&tft); // Тот самый буфер (спрайт) для меню
Preferences prefs;
WebServer server(80); 
...
int currentMode = 0; // 0: Осциллограф, 1: Меню, 3: WiFi, 4: Каналы, 5: Хинкали
int menuCursor = 0;
...
void updateMenuDisplay(bool forceRedraw = false);
...
void setup() {
  ...
  // Инициализируем буферный спрайт под размеры меню (высота 106 пикселей)
...
void loop() {
  ...
  // ИСПРАВЛЕНИЕ: Разделяем логику задержек, чтобы режим 0 мс работал моментально
  if (currentMode == 0) {
    if (!isFrozen) drawOscilloscope();
    if (graphSpeed == 0) {
      delayMicroseconds(10); // Разгрузка ядра при 0 мс, чтобы ESP32 не уходил в ребут
    }
  } else {
    if (currentMode == 1) {
    ...
    delay(16); // Делей анимаций применяется ТОЛЬКО в меню или таймере варки
  }
}

// --- ПЛАВНАЯ ОТРИСОВКА ЧЕРЕЗ БУФЕР (СПРАЙТ) ---
void updateMenuDisplay(bool forceRedraw) {
  static int lastRenderedCursor = -1;
  ...
  // Статичную рамку и заголовок рисуем прямо на экран только при смене страницы
  if (forceRedraw || lastRenderedCursor != menuCursor) {
    ...
    lastRenderedCursor = menuCursor;
  }

  // Локальные координаты целевого ползунка внутри Спрайта
  int activeVisualIndex = menuCursor - menuScrollOffset;
  targetVisualY = activeVisualIndex * 26 + 2;

  if (abs(currentVisualY - targetVisualY) > 0.2) {
    currentVisualY += (targetVisualY - currentVisualY) * 0.25;
  } else {
  ...
  // ИСПРАВЛЕНИЕ: Теперь всё меню рендерится внутри невидимого буфера menuSpr
  ...
  for (int i = 0; i < MAX_VISIBLE_ITEMS; i++) {
    ...
    if (itemIdx >= MENU_ITEMS_COUNT) break;

    int yTextPos = 5 + i * 26; // Позиция внутри буфера
    ...
    menuSpr.drawString(itemText, 15, yTextPos, 2);
  }

  if (menuScrollOffset > 0) menuSpr.drawString("^", menuSpr.width() - 15, 5, 2);
  if (menuScrollOffset + MAX_VISIBLE_ITEMS < MENU_ITEMS_COUNT) menuSpr.drawString("v", menuSpr.width() - 15, 85, 2);

  // Выталкиваем готовый кадр буфера на экран одной командой (Мерцание = 0%)
...
void drawKhinkaliScreen() {
  ...
  if (khinkaliTimerSec == 0 && khinkaliRunning) {
    tft.setTextColor(TFT_RED, (millis() % 400 < 200) ? TFT_BLACK : TFT_YELLOW);
...
void handleButtons() {
  ...
  if (rUP == LOW && lastStateUP == HIGH) {
    delay(30); 
    if (currentMode == 0) { currentMode = 1; menuCursor = 0; menuScrollOffset = 0; updateMenuDisplay(true); }
    else if (currentMode == 1) { menuCursor--; if (menuCursor < 0) { menuCursor = MENU_ITEMS_COUNT - 1; menuScrollOffset = MENU_ITEMS_COUNT - MAX_VISIBLE_ITEMS; } }
  ...
  if (rDOWN == LOW && lastStateDOWN == HIGH) {
    delay(30);
    if (currentMode == 0) { currentMode = 1; menuCursor = 0; menuScrollOffset = 0; updateMenuDisplay(true); }
    else if (currentMode == 1) { menuCursor++; if (menuCursor >= MENU_ITEMS_COUNT) { menuCursor = 0; menuScrollOffset = 0; } }
    ...
    else if (currentMode == 5) { currentMode = 1; khinkaliRunning = false; updateMenuDisplay(true); } 
  }
  lastStateDOWN = rDOWN;
  ...
  if (rLEFT == LOW && lastStateLEFT == HIGH) {
    ...
    if (currentMode == 0) { graphSpeed -= 5; if (graphSpeed < 0) graphSpeed = 0; } 
      ...
      else if (menuCursor == 6) { ledMode = (ledMode - 1 + 4) % 4; updateLED(); }
      else if (menuCursor == 7) { ledColorIndex = (ledColorIndex - 1 + 7) % 7; updateLED(); }
      else if (menuCursor == 8) { brightnessIndex = (brightnessIndex - 1 + 3) % 3; updateBrightness(); }
      else if (menuCursor == 11) { currentMode = 0; tft.fillScreen(TFT_BLACK); xPos = 0; }
      saveSettings();
    }
    else if (currentMode == 4) { if (pinsSubCursor < 3) { pinEnabled[pinsSubCursor] = !pinEnabled[pinsSubCursor]; saveSettings(); updatePinsMenuDisplay(); } else { currentMode = 1; updateMenuDisplay(true); } }
    else if (currentMode == 3) { if (wifiSubCursor == 0) { toggleWiFi(); updateWiFiMenuDisplay(); } else { currentMode = 1; updateMenuDisplay(true); } }
  ...
  if (rRIGHT == LOW && lastStateRIGHT == HIGH) {
    ...
    if (currentMode == 0) { graphSpeed += 5; if (graphSpeed > 30) graphSpeed = 30; }
      ...
      if (menuCursor == 0) { currentMode = 4; pinsSubCursor = 0; updatePinsMenuDisplay(); return; }
      if (menuCursor == 9) { currentMode = 3; wifiSubCursor = 0; updateWiFiMenuDisplay(); return; }
      if (menuCursor == 10) { currentMode = 5; khinkaliTimerSec = 420; khinkaliRunning = false; return; } 
      ...
      else if (menuCursor == 6) { ledMode = (ledMode + 1) % 4; updateLED(); }
      else if (menuCursor == 7) { ledColorIndex = (ledColorIndex + 1) % 7; updateLED(); }
      else if (menuCursor == 8) { brightnessIndex = (brightnessIndex + 1) % 3; updateBrightness(); }
      else if (menuCursor == 11) { currentMode = 0; tft.fillScreen(TFT_BLACK); xPos = 0; }
      saveSettings();
    }
    else if (currentMode == 4) { if (pinsSubCursor < 3) { pinEnabled[pinsSubCursor] = !pinEnabled[pinsSubCursor]; saveSettings(); updatePinsMenuDisplay(); } else { currentMode = 1; updateMenuDisplay(true); } }
...
void toggleWiFi() { wifiEnabled = !wifiEnabled; if (wifiEnabled) { WiFi.softAP("ESP32-Scope", "12345678"); server.begin(); } else { WiFi.softAPdisconnect(true); } saveSettings(); }
